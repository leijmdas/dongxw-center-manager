...
import cn.integriti.center.api.transformer.SysOrgTransformer;
...
public class SysOrgApiProvider implements SysOrgApiService {
	...
	public PageResult<SysOrgDTO> query(SysOrgQueryDTO qpDTO) {
		cn.integriti.center.biz.sys.domain.SysOrg.QueryParam qp = BeanMapper.getInstance().map(qpDTO, cn.integriti.center.biz.sys.domain.SysOrg.QueryParam.class);
	...
	public TreeNode<SysOrgDTO> tree(Integer corpId) {
		cn.integriti.center.core.support.tree.TreeNode<SysOrg> root = this.service.getRootNode(corpId);
		
		return transTree(root);
	}
	
	private TreeNode<SysOrgDTO> transTree(cn.integriti.center.core.support.tree.TreeNode<SysOrg> node){
		...
		if(node.getChildren() != null ) {
			...
			for(cn.integriti.center.core.support.tree.TreeNode<SysOrg> cNode:node.getChildren()) {
